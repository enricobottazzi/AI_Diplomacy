...
async def conduct_ndai_negotiations(
    ...
    """Run NDAI zone negotiations and store only agreed pacts."""
    ...
    for (proposer, accepter), text in agreed_statements.items():
        text = text.strip()
        logger.info(f"[NDAI] Pact disclosed: {proposer} <-> {accepter}: {text[:100]}...")
        game.add_message(
        ...
        if proposer in agents:
            agents[proposer].add_journal_entry(
                f"NDAI pact agreed with {accepter} in {phase}: {text[:100]}..."
            )
        if accepter in agents:
            agents[accepter].add_journal_entry(
                f"NDAI pact agreed with {proposer} in {phase}: {text[:100]}..."
            )

    logger.info(f"NDAI negotiation phase complete. {len(agreed_statements)} pact(s) disclosed.")
    return game_history
